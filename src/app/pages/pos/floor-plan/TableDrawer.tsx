...
import { guestLabel, tableDisplayLabel } from "../../../../i18n/utils";
...
export function TableDrawer({
  ...
  const td = (k: string, opts?: Record<string, unknown>) =>
    t(`floorPlan.tableDrawer.${k}`, opts as object);
  ...
  const drawerContent = sel && (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="p-5 flex items-start justify-between flex-shrink-0">
        <div>
          <h2 className="text-lg font-bold" style={{ color: C.text1 }}>
            {tableDisplayLabel(t, sel)}
          </h2>
          <div className="flex items-center gap-2 mt-1 text-sm flex-wrap" style={{ color: C.text2 }}>
            <span className="flex items-center gap-1">
              <Users size={13} /> {td("seats", { count: sel.seats })}
            </span>
            <span>|</span>
            <span className="flex items-center gap-1">
              <Clock size={13} /> {td("minutes", { count: 26 })}
            </span>
            {sel.status === "occupied" && (
              <>
                <span>|</span>
                <span style={{ color: C.occupied.text }}>{td("occupied")}</span>
              </>
            )}
            {sel.status === "reserved" && (
              <>
                <span>|</span>
                <span style={{ color: C.reserved.text }}>{td("reserved")}</span>
              </>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="cursor-pointer" style={{ color: C.text3 }}>
            <MoreVertical size={18} />
          </button>
          <button onClick={onClose} className="cursor-pointer" style={{ color: C.text3 }}>
            <X size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {sel.status === "available" && (
          <div className="p-5 flex items-center justify-center" style={{ minHeight: 120 }}>
            <span style={{ color: C.text3 }}>{td("noOrders")}</span>
          </div>
        )}
        {sel.status === "reserved" && (
          <div className="p-5">
            <div className="space-y-2">
              <div className="flex justify-between">
                <span style={{ color: C.text2 }}>{td("guest")}</span>
                <span style={{ color: C.text1 }}>{guestLabel(t, sel)}</span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: C.text2 }}>{td("time")}</span>
                <span style={{ color: C.text1 }}>{sel.reservationTime}</span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: C.text2 }}>{td("partySize")}</span>
                <span style={{ color: C.text1 }}>{sel.seats}P</span>
              </div>
            </div>
            <div className="mt-4 pt-4 flex flex-col items-center" style={{ borderTop: `1px solid ${C.border}` }}>
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[11px] uppercase tracking-wider" style={{ color: C.text3 }}>
                  {td("reservationQr")}
                </span>
              </div>
              <div className="rounded-xl p-3 bg-white shadow-sm">
                <QRCodeSVG
                  value={JSON.stringify({
                    ...
                    table: tableDisplayLabel(t, sel),
                    guest: guestLabel(t, sel),
                    party: sel.seats,
        ...
        <div className="p-5 flex-shrink-0">
          <div className="flex justify-between mb-1">
            <span style={{ color: C.text2 }}>
              {td("orderTotal", {
                count: sel.orderItems?.filter((it) => it.qty > 0).reduce((a, b) => a + b.qty, 0) ?? 0,
              })}
            </span>
            <span style={{ color: C.text1 }}>{"\u20A9"}{sel.revenue.toLocaleString("ko-KR")}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-bold" style={{ color: C.text1 }}>
              {td("paymentTotal")}
            </span>
            <span className="font-bold text-lg" style={{ color: C.text1 }}>
              {"\u20A9"}
              {sel.revenue.toLocaleString("ko-KR")}
            </span>
          </div>
              ...
              onClick={() => {
                ...
                navigate("/pos/payment", {
                  state: {
                    ...
                    tableLabel: tableDisplayLabel(t, sel),
                    returnTo: "/pos",
                  },
                });
              }}
      ...
      {sel.status === "reserved" && (
        <div className="p-5 flex-shrink-0">
          <div className="flex gap-2">
            ...
            <button
              className="flex-1 py-2.5 rounded-lg text-sm font-semibold cursor-pointer"
              style={{ background: C.raised, color: C.text1, border: `1px solid ${C.border}` }}
            >
              {td("cancel")}
            </button>
          </div>
