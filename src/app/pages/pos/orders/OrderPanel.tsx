...
export function OrderPanel(props: OrderPanelProps) {
  ...
  const todayBills: Array<{
    id: string; table: string; time: string; krw: number; usd: number; method: string;
    items: Array<{ name: string; qty: number; price: number; currency: "domestic" | "foreign" }>;
  }> = [
    { id: "B-1042", table: "T8", time: "08:42", krw: 38000, usd: 0, method: "Cash", items: [
      { name: "Kimchi Stew", qty: 2, price: 12000, currency: "domestic" },
      { name: "Rice", qty: 2, price: 2000, currency: "domestic" },
      { name: "Soju", qty: 1, price: 10000, currency: "domestic" },
    ]},
    { id: "B-1043", table: "T3", time: "09:15", krw: 0, usd: 42.5, method: "Credit", items: [
      { name: "Cheeseburger", qty: 2, price: 14.0, currency: "foreign" },
      { name: "Fries", qty: 1, price: 6.5, currency: "foreign" },
      { name: "Cola", qty: 2, price: 4.0, currency: "foreign" },
    ]},
    { id: "B-1044", table: "T6", time: "10:03", krw: 124000, usd: 18.0, method: "Mix", items: [
      { name: "Galbi Set", qty: 2, price: 45000, currency: "domestic" },
      { name: "Bibimbap", qty: 1, price: 14000, currency: "domestic" },
      { name: "Beer Pitcher", qty: 1, price: 20000, currency: "domestic" },
      { name: "Tip", qty: 1, price: 18.0, currency: "foreign" },
    ]},
    { id: "B-1045", table: "BAR1", time: "10:51", krw: 22000, usd: 0, method: "Cash", items: [
      { name: "Highball", qty: 2, price: 11000, currency: "domestic" },
    ]},
    { id: "B-1046", table: "T1", time: "11:28", krw: 0, usd: 96.75, method: "Credit", items: [
      { name: "Steak", qty: 2, price: 38.0, currency: "foreign" },
      { name: "Caesar Salad", qty: 1, price: 12.75, currency: "foreign" },
      { name: "Wine Glass", qty: 1, price: 8.0, currency: "foreign" },
    ]},
    { id: "B-1047", table: "T11", time: "12:09", krw: 64500, usd: 0, method: "Cash", items: [
      { name: "Bulgogi", qty: 2, price: 22000, currency: "domestic" },
      { name: "Japchae", qty: 1, price: 16000, currency: "domestic" },
      { name: "Makgeolli", qty: 1, price: 4500, currency: "domestic" },
    ]},
    { id: "B-1048", table: "T4", time: "12:47", krw: 88000, usd: 24.0, method: "Mix", items: [
      { name: "Pork Belly", qty: 3, price: 24000, currency: "domestic" },
      { name: "Soju", qty: 1, price: 16000, currency: "domestic" },
      { name: "Service Charge", qty: 1, price: 24.0, currency: "foreign" },
    ]},
  ];
  ...
  const [editConfirm, setEditConfirm] = useState<
    | { type: "qty"; id: string; name: string; from: number; to: number }
    | { type: "remove"; id: string; name: string; qty: number }
    | null
  >(null);
  ...
  const interceptQty = (id: string, value: number) => {
    ...
    if (item.ordered) {
      ...
      if (next === 0) {
        setEditConfirm({ type: "remove", id, name: item.name, qty: item.qty });
      } else {
        setEditConfirm({ type: "qty", id, name: item.name, from: item.qty, to: next });
      }
      return;
    }
    setItemQty(id, value);
    ...
    >
        ...
        <div className="flex items-center justify-between gap-2">
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="flex items-center gap-1.5">
                            ...
                            <span className={`text-[0.6875rem] ${selectedTable === table.id ? "text-white/80" : tc.subtext}`}>{table.seats} seats</span>
                          </span>
                        </button>
                      );
                    })}
          ...
          {canViewHistory && (
            <button
              ...
              title="View today's bill history"
      ...
      <div ref={orderItemsRef} className="flex-1 overflow-y-auto">
        {currentOrder.length === 0 ? (
          ...
          <OrderItemsTable
            items={currentOrder.filter((i) => !i.deleted).map((i) => ({ id: i.id, name: i.name, qty: i.qty, price: i.price, ordered: i.ordered, currency: i.currency }))}
      ...
      <div className={`border-t ${tc.borderHalf} p-2 space-y-0.5`}>
        <div className="flex justify-between text-[0.75rem] font-semibold">
          <span className={tc.heading}>Domestic (₩):</span>
          <span className={tc.heading}>₩{Math.round(totalKrw).toLocaleString()}</span>
        </div>
        <div className="flex justify-between text-[0.75rem] font-semibold">
          <span className={tc.heading}>Foreign ($):</span>
        ...
        >
          ...
          >
            <div className="text-base font-semibold mb-1">
              {editConfirm.type === "remove" ? "Remove Item" : "Update Quantity"}
            </div>
            <div className={`text-sm mb-4 ${tc.subtext}`}>
              {editConfirm.type === "remove"
                ? <>Remove <span className={tc.heading}>{editConfirm.name}</span> (×{editConfirm.qty}) from this order?</>
                : <><span className={tc.heading}>{editConfirm.name}</span>: ×{editConfirm.from} → ×{editConfirm.to}</>
              }
            </div>
            ...
            <div className={`flex items-center justify-between px-5 py-3 border-b ${tc.borderHalf}`}>
              <div>
                <div className="text-base font-semibold flex items-center gap-2">
                  <History className="w-4 h-4" /> Today's Bill History
                </div>
                <div className={`text-[0.75rem] ${tc.subtext}`}>
                  {todayBills.length} bills · ₩{todayKrw.toLocaleString()} · ${todayUsd.toFixed(2)}
                </div>
              </div>
              <button
                onClick={() => setHistoryOpen(false)}
                      ...
                      <Fragment key={b.id}>
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            {(() => {
                              const t = TABLES.find((x) => x.id === b.table);
                              return t ? `${t.floor.toLowerCase()} - ${b.table.toLowerCase()}` : b.table;
                            })()}
                          </td>
                          <td className="px-3 py-2">{b.time}</td>
                          <td className="px-3 py-2">{b.method}</td>
                          <td className="px-3 py-2 text-right">
                            {b.krw > 0 && <span className="text-blue-600">₩{b.krw.toLocaleString()}</span>}
                                ...
                                <table className="w-full text-[0.8125rem]">
                                  <thead className={tc.subtext}>
                                    <tr className={`border-b ${tc.borderHalf}`}>
                                      ...
                                      <th className="text-right px-3 py-1.5 font-medium w-20">Subtotal</th>
                                    </tr>
                                  </thead>
                                  <tbody className={`divide-y ${tc.borderHalf}`}>
                                    {b.items.map((it, i) => {
                                      const sym = it.currency === "domestic" ? "₩" : "$";
                                      const color = it.currency === "domestic" ? "text-blue-600" : "text-red-600";
                                      const fmt = (n: number) => it.currency === "domestic" ? `₩${Math.round(n).toLocaleString()}` : `$${n.toFixed(2)}`;
                                      void sym;
                                      return (
                                        <tr key={`${b.id}-${i}`}>
      ...
      {confirmOpen && (
        ...
        >
          <div
            ...
            className={`w-[90vw] max-w-md rounded-lg shadow-xl border p-5 ${tc.isDark ? "bg-[#2a2d35] border-slate-700 text-slate-100" : "bg-white border-slate-200 text-slate-900"}`}
          >
            <div className="text-base font-semibold mb-1">Confirm Order</div>
            <div className={`text-sm mb-3 ${tc.subtext}`}>
              {pendingCount} new {pendingCount === 1 ? "item" : "items"} for {selectedTableInfo?.label || selectedTable}
            </div>
            <div className={`max-h-60 overflow-y-auto rounded border ${tc.borderHalf} divide-y ${tc.borderHalf} mb-4`}>
              {pendingItems.map((it) => {
                const tag = it.deleted ? "Remove" : !it.ordered ? "New" : `Qty ${it.origQty} → ${it.qty}`;
                ...
                return (
                  <div key={it.id} className="flex items-center justify-between px-3 py-2 text-sm">
                    ...
                    <span className={`shrink-0 ml-3 ${tc.subtext}`}>
                      ×{it.qty} · {it.currency === "domestic" ? `₩${Math.round(it.price * it.qty).toLocaleString()}` : `$${(it.price * it.qty).toFixed(2)}`}
                    </span>
                  </div>
                );
              })}
